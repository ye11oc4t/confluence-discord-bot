...
def _trunc(text: str, n: int = 300) -> str:
    ...
    import re
    text = re.sub(r"<[^>]+>", "", text).strip()   # 간단 HTML 제거
...
def format_page_created(p, event):
    ...
    return {
        ...
        "url":         _page_url(page),
        "color":       COLOR["page_created"],
        "fields": [
            {"name": "스페이스", "value": _space(page), "inline": True},
        ],
        "author":  _user(p),
        "footer":  {"text": f"Page Created • {_space(page)}"},
    }
...
def format_page_updated(p, event):
    ...
    return {
        ...
        "url":         _page_url(page),
        "color":       COLOR["page_updated"],
        "fields": [
            {"name": "스페이스", "value": _space(page),   "inline": True},
            {"name": "버전",     "value": _version(page), "inline": True},
        ],
        "author":  _user(p),
        "footer":  {"text": f"Page Updated • {_space(page)}"},
    }

def format_page_trashed(p, event):
    page = p.get("page", {})
    return {
        "title":  f"🗑️ 페이지 삭제(휴지통): {page.get('title', '(제목 없음)')}",
        "url":    _page_url(page),
        "color":  COLOR["page_trashed"],
        "fields": [{"name": "스페이스", "value": _space(page), "inline": True}],
        "author": _user(p),
        "footer": {"text": f"Page Trashed • {_space(page)}"},
    }

def format_page_restored(p, event):
    page = p.get("page", {})
    return {
        "title":  f"♻️ 페이지 복원: {page.get('title', '(제목 없음)')}",
        "url":    _page_url(page),
        "color":  COLOR["page_restored"],
        "fields": [{"name": "스페이스", "value": _space(page), "inline": True}],
        "author": _user(p),
        "footer": {"text": f"Page Restored • {_space(page)}"},
    }

def format_page_moved(p, event):
    page       = p.get("page", {})
    old_parent = p.get("previousParent", {})
    new_parent = p.get("page", {}).get("parent", {})
    old_title  = old_parent.get("title", "?") if old_parent else "?"
    new_title  = new_parent.get("title", "?") if new_parent else "?"
    return {
        "title":  f"📂 페이지 이동: {page.get('title', '(제목 없음)')}",
        "url":    _page_url(page),
        "color":  COLOR["page_moved"],
        "fields": [
            {"name": "스페이스",    "value": _space(page), "inline": True},
            {"name": "이전 위치",   "value": old_title,    "inline": True},
            {"name": "새 위치",     "value": new_title,    "inline": True},
        ],
        "author": _user(p),
        "footer": {"text": f"Page Moved • {_space(page)}"},
    }
...
def format_comment_created(p, event):
    ...
    return {
        ...
        "fields": fields,
        "author": _user(p),
        "footer": {"text": f"Comment • {_space(page)}"},
    }
...
def format_comment_updated(p, event):
    ...
    return {
        "title":  "✏️ 댓글 수정",
        "url":    _page_url(page),
        "color":  COLOR["comment_updated"],
        "fields": [
            {"name": "페이지",  "value": page.get("title", "-"), "inline": True},
            {"name": "스페이스","value": _space(page),            "inline": True},
        ],
        "author": _user(p),
        "footer": {"text": f"Comment Updated • {_space(page)}"},
    }
...
def format_comment_removed(p, event):
    ...
    return {
        "title":  "🗑️ 댓글 삭제",
        "url":    _page_url(page),
        "color":  COLOR["comment_removed"],
        "fields": [
            {"name": "페이지",  "value": page.get("title", "-"), "inline": True},
            {"name": "스페이스","value": _space(page),            "inline": True},
        ],
        "author": _user(p),
        "footer": {"text": f"Comment Removed • {_space(page)}"},
    }
...
def format_attachment_created(p, event):
    ...
    return {
        "title":  f"📎 첨부파일 업로드: {attachment.get('title', '?')}",
        "url":    _page_url(page),
        "color":  COLOR["attachment_created"],
        "fields": [
            {"name": "페이지",  "value": page.get("title", "-"), "inline": True},
            {"name": "스페이스","value": _space(page),            "inline": True},
        ],
        "author": _user(p),
        "footer": {"text": f"Attachment • {_space(page)}"},
    }

def format_space_created(p, event):
    space = p.get("space", {})
    return {
        "title":  f"🗂️ 새 스페이스 생성: {space.get('name', '?')}",
        "color":  COLOR["space_created"],
        "fields": [{"name": "키", "value": space.get("key", "-"), "inline": True}],
        "author": _user(p),
        "footer": {"text": "Space Created"},
    }
...
def format_default(p, event):
    ...
    return {
        "title":       f"⚡ `{event}`",
        "color":       COLOR["default"],
        "description": f"Confluence에서 `{event}` 이벤트 발생",
        "author":      _user(p),
        "footer":      {"text": event},
    }
